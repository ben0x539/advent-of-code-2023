...
impl Trace {
    ...
    fn record(&mut self, Ray { pos: V2(x, y), dir: V2(dx, dy) }: Ray) -> bool {
        ...
        let b =
            ...
            //    _ => 0,
            //};
            ((dx+3*dx*dx)/2 + (dy+3*dy*dy)*2) as u8;
        let new = b & *r == 0;
...
fn trace(grid: &Grid, pos: V2, dir: V2, escaped: &mut [&mut Vec<bool>; 4]) -> i32 {
    let mut rays = vec![Ray { pos, dir }];
    //let mut next_rays = vec![];
    let mut paths = Trace::new_for(grid);

    loop {
        let Some(mut ray) = rays.pop() else { break; };
        loop {
            ray.step();
            ...
            //}
            if ray.pos.0 < 0 {
                escaped[0][ray.pos.1 as usize] = true;
                break;
            }

            if ray.pos.0 >= grid.width {
                escaped[1][ray.pos.1 as usize] = true;
                break;
            }

            if ray.pos.1 < 0 {
                escaped[2][ray.pos.0 as usize] = true;
                break;
            }

            if ray.pos.1 >= grid.width {
                escaped[3][ray.pos.0 as usize] = true;
                break;
            }
...
fn main() -> Result<()> {
    ...
    //dbg!(energized);
    let mut escaped_left = vec![false; grid.height as usize];
    let mut escaped_right = vec![false; grid.height as usize];
    let mut escaped_up = vec![false; grid.width as usize];
    let mut escaped_down = vec![false; grid.width as usize];
    let mut escaped = [&mut escaped_left, &mut escaped_right, &mut escaped_up, &mut escaped_down];
    let mut max = 0;
    let mut c = 0;
    for y in 0..grid.height {
        if !escaped[0][y as usize] {
            c += 1;
            max = i32::max(max, trace(&grid, V2(-1, y), EAST, &mut escaped));
        }
        if !escaped[1][y as usize] {
            c += 1;
            max = i32::max(max, trace(&grid, V2(grid.width, y), WEST, &mut escaped));
        }
        //max = i32::max(max, trace(&grid, V2(-1, y), EAST, &mut escaped));
        //max = i32::max(max, trace(&grid, V2(grid.width, y), WEST, &mut escaped));
    }
    for x in 0..grid.width {
        if !escaped[2][x as usize] {
            c += 1;
            max = i32::max(max, trace(&grid, V2(x, -1), SOUTH, &mut escaped));
        }
        if !escaped[3][x as usize] {
            c += 1;
            max = i32::max(max, trace(&grid, V2(x, grid.height), NORTH, &mut escaped));
        }
        //max = i32::max(max, trace(&grid, V2(x, -1), SOUTH, &mut escaped));
        //max = i32::max(max, trace(&grid, V2(x, grid.height), NORTH, &mut escaped));
    }
